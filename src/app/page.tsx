  ...
  return (
    <div className="vhi-home">
      ...
      <section className="home-section home-shell pb-12">
        ...
        <div className="stay-grid compact-stay-grid mt-7">{stays.map((s: any) => <StayCard key={s.id} stay={s} />)}</div>
      </section>
      ...
      <section className="home-section home-standard-compact"><div className="home-shell"><div className="standard-compact-grid"><div className="standard-compact-image"><Photo src={bySection("standard")[0]?.image ?? stays[1]?.images?.[1]} alt="The Vrindavan Holiday Inn standard" className="h-full w-full" /></div><div className="standard-compact-copy"><span className="home-eyebrow"><BadgeCheck size={13} /> THE VHI STANDARD</span><h2>{bySection("standard")[0]?.title ?? "A beautiful house is only the beginning."}</h2><p>{bySection("standard")[0]?.body ?? "The stay should feel effortless from the moment you arrive."}</p><div className="standard-mini-grid">{standard.slice(0,6).map((x: any, i: number) => <div key={x.id}><span>{[ShieldCheck, ChefHat, Heart, BedDouble, Users, CalendarDays][i] ? (() => { const I=[ShieldCheck, ChefHat, Heart, BedDouble, Users, CalendarDays][i]; return <I size={17}/>; })() : null}</span><div><b>{x.title}</b><small>{x.subtitle || x.body}</small></div></div>)}</div></div></div></div></section>
      ...
      {testimonials.length > 0 && <section className="home-section home-shell pb-12"><div className="home-section-head"><div><span className="home-eyebrow"><Star size={13}/> GUEST WORDS</span><h2>What guests remember</h2><p>Thoughtful stays, shared by the people who stayed with us.</p></div></div><div className="testimonial-rail mt-7">{testimonials.map((t:any)=><article key={t.id} className="testimonial-card"><div className="testimonial-stars">{"★".repeat(Math.max(1, Math.min(5, Number(t.rating)||5)))}</div><p>“{t.quote}”</p><div><b>{t.name}</b>{t.location&&<span>{t.location}</span>}{t.source === "GOOGLE" && <small>Google review</small>}</div></article>)}</div></section>}
      ...
      <section className="home-section home-shell pb-12"><div className="stats-compact"><div><b>{stays.length}</b><span>homes in Vrindavan</span></div><div><b>24/7</b><span>local support</span></div><div><b>4.8+</b><span>guest ratings</span></div><div><b>1</b><span>thoughtful destination</span></div></div></section>
